...
\paragraph{Architecture.} TAMGSlotEncoder (Figure~\ref{fig:tamg}) replaces the oracle with a learned CNN backbone (4 conv layers, 64-dim features) followed by learned slot queries with spatial anchoring. Each frame pair $(F_t, F_{t+1})$ produces an $(N, 19)$ structured slot tensor:
