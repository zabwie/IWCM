...
\begin{enumerate}
  ...
  \item \textbf{Recovery from degraded rollouts.} Even when the rollout model is deliberately crippled (95$\times$ higher MSE), the warm-started IWCM solver recovers trajectories with energy below ground truth -- the solver acts as a constraint satisfaction filter that pulls drifted predictions back to validity.
...
The IWCM energy function $E_\theta(z_0, A, Z)$ is trained to assign low energy to physically valid trajectories and high energy to invalid ones. The choice of \emph{which} invalid trajectories to train on determines what the model learns. Random corruptions teach the statistical regularities of the training distribution; a targeted curriculum forces the model to learn causal structure.
...
\paragraph{Generalization to video.} The TAMGSlotEncoder replaces the oracle in the grid world setting (Section~\ref{sec:exp_tamg}), but the DM Control experiments still use oracle-structured features. Extending TAMG to continuous control domains (cartpole, walker, quadruped) requires adapting the visual encoder to rendered MuJoCo frames or real camera input. The structured self-supervision framework (velocity, identity, type) is domain-agnostic and should transfer directly. We consider this the highest-priority next step and expect to complete the continuous-control TAMG integration within two weeks of public release, using the existing DM Control wrapper and encoder infrastructure.
