...
\begin{enumerate}
  \item \textbf{IWCM eliminates drift by construction.} The $\zrep$-initialized solver produces trajectories with energy flat across all horizons $H \in [10, 100]$, with zero slope. This is Proposition~\ref{prop:drift_elim} made empirical.
  ...
  \item \textbf{Compositional corruption causes law learning.} The 0.952 AUROC on cross-surface generalization (Table~\ref{tab:cross_surface}) proves that IWCM learns causal invariants, not surface statistics.
\end{enumerate}
...
\paragraph{Generalization to video.} The current experiments use oracle-structured slot encoders that extract physics state (position, velocity) directly. Extending to learned visual encoders (pixels $\to$ slots $\to$ IWCM) requires the TAMG Validator Committee framework, which remains future work.
...
All code, data, and experiment reproduction scripts are available at \url{https://github.com/anonymous/iwcm}.
