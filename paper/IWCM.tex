...
The energy metric measures constraint satisfaction under the learned discriminator, not physical state error. To characterize this relationship, we compute the state MSE (position and velocity slot channels, normalized) between each method's predicted trajectory and the simulator ground truth.

\begin{table}[h]
\centering
\caption{Energy gap $\Delta E$ vs physical state MSE (position + velocity channels) across horizons on DM Control cartpole. Lower energy does not guarantee lower physical error.}
...
Three observations emerge. First, the rollout MLP achieves the lowest MSE ($2\times 10^{-5}$ at $H=100$) with a small positive energy drift, confirming that well-trained autoregressive models can be accurate on simple physics. Second, the $\zrep$ solver achieves the most negative energy ($-0.698$) but at higher physical MSE ($8.4\times 10^{-4}$) -- optimizing the learned energy does not monotonically improve physical accuracy. Third, the warm-start solver recovers most of the rollout's physical accuracy (MSE $1.6\times 10^{-4}$) while still achieving negative energy ($-0.599$), demonstrating that combining a rollout initialization with IWCM refinement provides the best trade-off.

The key implication: learned energy and physical accuracy are correlated but not equivalent. The energy function detects constraint violations that correlate with physical invalidity, but aggressive optimization can produce low-energy trajectories that deviate from true dynamics. This reinforces the framing of IWCM as a constraint-satisfaction filter rather than a dynamics predictor -- its strength is eliminating autoregressive chain drift in the learned energy landscape, with physical accuracy depending on energy calibration quality.
...
The solver is 0.13 ms per trajectory in this implementation. IWCM is not faster than a tiny rollout MLP (0.008 ms for 100 steps), but it removes autoregressive dependency while remaining computationally cheap. As a point of reference, DreamerV3's RSSM rollout is estimated at $\sim$200 ms for 100 steps in its optimized framework \citep{hafner2023dreamerv3}. Direct runtime comparison depends heavily on implementation details, environment interface, and hardware. The key architectural point is that IWCM avoids sequential latent updates entirely; the raw speed advantage over an optimized DreamerV3 implementation may be smaller than this architectural estimate suggests.
...
\begin{enumerate}
  ...
  \item \textbf{Recovery is robust under the learned energy.} Even from a deliberately degraded rollout (95$\times$ higher MSE), the warm-started solver finds trajectories with energy below ground truth. Note: lower learned energy indicates better constraint satisfaction by the learned metric, not necessarily higher physical accuracy. Table~\ref{tab:energy_mse} quantifies this gap: on cartpole, the $\zrep$ solver achieves $\Delta E = -0.698$ at $H=100$ but with state MSE $8.4\times 10^{-4}$, while the rollout MLP has $+0.046$ energy drift but MSE $2\times 10^{-5}$. The energy metric and physical accuracy are correlated but not equivalent.
...
\begin{itemize}
  ...
  \item \textbf{Recovery from degraded predictions}: Even from a rollout with $95\times$ higher error, IWCM recovers trajectories below ground-truth energy (Table~\ref{tab:energy_mse}) -- though lower energy is not equivalent to lower physical state error.
