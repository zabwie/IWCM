...
# ─── STEP 4: Targeted adversarial IWCM training ──────────────────────────
print("\nSTEP4: Targeted adversarial IWCM...")
model = SlotIWCMEnergy(d_slot=d, d_action=11, hidden_dim=128, num_slots=N).to(DEVICE)
opt = torch.optim.Adam(model.parameters(), lr=3e-3)

def targeted_corrupt(vZ, vz0, vA, cur_model):
    """Find most vulnerable slot per trajectory, perturb only that slot."""
    B, H_in, N_in, d_in = vZ.shape
    cZ = vZ.clone()
    for b in range(B):
        best_slot, best_e = 0, float('-inf')
        Z_test = vZ[b:b+1].clone()
        for slot in range(N_in):
            Z_test[0, :, slot] += operators[np.random.randint(0, n_ops)].unsqueeze(0) * 0.3
            with torch.no_grad():
                e = cur_model(vz0[b:b+1], vA[b:b+1], Z_test).item()
            Z_test[0, :, slot] = vZ[b, :, slot].clone()  # reset
            if e > best_e:
                best_slot, best_e = slot, e
        cZ[b, np.random.randint(0, max(1, H_in // 2)):, best_slot] += operators[np.random.randint(0, n_ops)] * 0.4
    return cZ
...
for ep in range(400):
    ...
    cZ = targeted_corrupt(vZ, vz0, vA, model)
    d_c = disagreement_sigmoid(cZ, vz0, vA)
    d_v = disagreement_sigmoid(vZ, vz0, vA)
    
    ev = model(vz0, vA, vZ)
    ec = model(vz0, vA, cZ)
    w = (d_c.mean() / max(d_v.mean(), 1e-5)).detach().clamp(0.5, 3.0)
    ...
    opt.step()
    if (ep + 1) % 100 == 0:
        print(f"  ep{ep+1}: ev={ev.mean():+.2f} ec={ec.mean():+.2f} w={w.item():.3f}")
